...
const FormInput = ({
    ...
    return (
        <View className={cn('gap-1.5', containerClassName)}>
            {label && (
                <Text className='text-font-primary font-semibold text-sm'>{label}</Text>
            )}
            <View className={cn(
                'flex-row items-center bg-orange-second rounded-2xl px-4 gap-3',
                error && 'border border-red-500',
            )}>
                {leftIcon && <View className='opacity-60'>{leftIcon}</View>}
                <TextInput
                    className={cn('flex-1 text-font-primary text-base py-4', className)}
                    placeholderTextColor="#c4836a"
                    {...props}
                />
                {rightIcon && (
